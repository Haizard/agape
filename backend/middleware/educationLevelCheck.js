  ...
  return async (req, res, next) => {
    try {
      ...
      if (student.educationLevel !== requiredLevel) {
        logToFile(`Education level mismatch: Student ${studentId} has ${student.educationLevel} but ${requiredLevel} is required`);
      ...
      logToFile(`Error in education level check: ${err.message}`);
  ...
  return async (req, res, next) => {
    try {
      ...
      if (classObj.educationLevel !== requiredLevel) {
        logToFile(`Education level mismatch: Class ${classId} has ${classObj.educationLevel} but ${requiredLevel} is required`);
