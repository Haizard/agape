...
OLevelResultSchema.pre('save', function(next) {
  ...
  if (this.marksObtained !== undefined) {
    // O-LEVEL grading logic based on Tanzania's CSEE system
    ...
    console.log(`[O-LEVEL] Calculated grade for marks ${this.marksObtained}: ${this.grade} (${this.points} points)`);
    console.log(`[O-LEVEL] Result for student ${this.studentId}, subject ${this.subjectId}, exam ${this.examId}`);
  }
...
OLevelResultSchema.pre('save', async function(next) {
  try {
    ...
    if (existingResult) {
      console.log(`[O-LEVEL] Found existing result for student ${this.studentId} in exam ${this.examId} for subject ${existingResult.subjectId}`);
      console.log(`[O-LEVEL] Ensuring marks are not duplicated from subject ${existingResult.subjectId} to ${this.subjectId}`);

      // If the marks are exactly the same, it might be a duplicate
      if (existingResult.marksObtained === this.marksObtained) {
        console.warn(`[O-LEVEL] Potential duplicate marks detected: ${this.marksObtained} for student ${this.studentId} across subjects ${existingResult.subjectId} and ${this.subjectId}`);
